/**
 * dsh-hello-plugin — a minimal dsh bundle plugin (MVP) that proves it loaded.
 *
 * Loaded by the profile Loader as the `hello-plugin` entry, it:
 *  1. waits for the web surface's `webServer` service (declared via inject),
 *  2. registers an exact HTTP route `GET /hello-plugin` whose JSON response is
...
export const ROUTE_PATH = '/hello-plugin'

export function apply(ctx) {
  const logger = ctx.logger('hello-plugin')
  logger.info('dsh-hello-plugin applied — registering %s', ROUTE_PATH)

  // Route registration is the visible proof: after the profile boots,
  // http://<dsh-web-host>:<port>/hello-plugin answers with this payload.
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: (_req, res) => {
      ...
      res.end(JSON.stringify({
        plugin: name,
        status: 'loaded',
        message: 'Hello from the dsh hello plugin — the bundle loaded successfully.',
        route: ROUTE_PATH,
