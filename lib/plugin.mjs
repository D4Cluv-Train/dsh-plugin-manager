/**
 * dsh-plugin-manager — a minimal dsh bundle plugin (MVP) that proves it loaded.
 *
...
export function apply(ctx) {
  ...
  logger.info('dsh-plugin-manager applied — registering %s', ROUTE_PATH)
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: (_req, res) => {
      ...
      res.end(JSON.stringify({
        plugin: name,
        status: 'loaded',
        message: 'Hello from dsh-plugin-manager — the bundle loaded successfully.',
        route: ROUTE_PATH,
